...
class ShardWriter {
    ...
    private fun insertMetadata(connection: Connection, jarSha256: String, extracted: ExtractedJar) {
        connection.prepareStatement(
            ...
            statement.setInt(3, Research4JarVersions.EXTRACTOR)
            statement.setInt(4, Research4JarVersions.SCHEMA)
    ...
    private fun dev.research4jar.indexer.extract.ExtractedAnnotation.methodKey() =
        MethodKey(classFqn, requireNotNull(methodName), requireNotNull(methodDescriptor))

    private fun dev.research4jar.indexer.extract.ExtractedCondition.methodKey() =
        MethodKey(classFqn, requireNotNull(methodName), requireNotNull(methodDescriptor))

    private fun dev.research4jar.indexer.extract.ExtractedStringConstant.methodKey() =
        MethodKey(classFqn, requireNotNull(methodName), requireNotNull(methodDescriptor))
