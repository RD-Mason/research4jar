...
func runStatusCommand(args []string) {
	...
	statusOptions := query.ProjectStatusOptions{}
	for index := 0; index < len(args); index++ {
		argument := args[index]
		switch argument {
		case "--project-dir", "--home", "--format":
			...
			index = next
		case "--check-classpath":
			statusOptions.CheckClasspath = true
		default:
			fail("invalid_arguments", "unknown option: "+argument, 2)
		}
	}
	response, err := query.ProjectStatusWithOptions(opts.projectDir, opts.home, statusOptions)
	if err != nil {
...
func noProjectIndexMessage(projectDir string) string {
	command := "research4jar index"
	if projectDir != "" && projectDir != "." {
		command += " --project-dir " + strconv.Quote(projectDir)
	}
...
func unknownCommandMessage(command string) string {
	...
	suggestions := commandSuggestions(command)
...
func commandSuggestions(command string) []string {
	command = strings.ToLower(strings.TrimSpace(command))
	if command == "" {
		return nil
	}
	candidates := knownTopLevelCommands()
	type scored struct {
		command string
		score   int
	}
	scoredCandidates := []scored{}
	threshold := maxInt(2, len(command)/3)
	for _, candidate := range candidates {
		score := commandSuggestionScore(command, candidate)
		if score <= threshold {
			scoredCandidates = append(scoredCandidates, scored{command: candidate, score: score})
		}
	}
	sort.Slice(scoredCandidates, func(i, j int) bool {
		...
		return scoredCandidates[i].command < scoredCandidates[j].command
	})
	limit := minInt(3, len(scoredCandidates))
	suggestions := make([]string, limit)
	for index := 0; index < limit; index++ {
		suggestions[index] = scoredCandidates[index].command
	}
	return suggestions
}
...
func knownTopLevelCommands() []string {
	...
	for command := range queryCommands {
...
func commandSuggestionScore(input, candidate string) int {
	...
	compactInput := strings.ReplaceAll(input, "-", "")
	compactCandidate := strings.ReplaceAll(candidate, "-", "")
	if compactInput == compactCandidate {
		return 0
	}
	if strings.HasPrefix(compactCandidate, compactInput) {
		return 1
	}
	...
	for leftIndex := 1; leftIndex <= len(left); leftIndex++ {
		...
		for rightIndex := 1; rightIndex <= len(right); rightIndex++ {
			...
			current[rightIndex] = minInt(
				...
				minInt(current[rightIndex-1]+1, previous[rightIndex-1]+cost),
			)
		}
...
func printProjectStatusText(response query.ProjectStatusResponse) {
	...
	if response.ClasspathCheck != nil {
		check := response.ClasspathCheck
		fmt.Println("Classpath check:", check.Checked)
		if check.CurrentFingerprint != "" {
			fmt.Println("Current classpath fingerprint:", check.CurrentFingerprint)
		}
		if check.IndexedFingerprint != "" {
			fmt.Println("Indexed classpath fingerprint:", check.IndexedFingerprint)
		}
		if check.DependencyResolution != "" {
			fmt.Println("Classpath resolution:", check.DependencyResolution)
		}
		if check.JarsResolved > 0 {
			fmt.Println("Classpath jars resolved:", check.JarsResolved)
		}
		if check.JarsUnique > 0 {
			fmt.Println("Classpath unique jars:", check.JarsUnique)
		}
		if check.Error != "" {
			fmt.Println("Classpath check error:", check.Error)
		} else {
			fmt.Println("Classpath up to date:", check.UpToDate)
		}
	}
