...
- The session layout version is now 8 (adds shard/range bookkeeping, delta depth, trigram shadows for strings/classes/methods, a method-owner string index, compact ordered INTEGER source-shard keys, and derived method symbols); existing sessions rebuild on first index.
